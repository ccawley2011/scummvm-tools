...
#define OPCODEB(x) _OPCODEB(Script_v4, x)

const int Script_v4::_goblinFuncLookUp[][2] = {
	{0, 0},
	{1, 1},
	{2, 2},
	{4, 3},
...
Script_v4::Script_v4(byte *totData, uint32 totSize, ExtTable *extTable) :
	Script_v3(totData, totSize, extTable) {
...
void Script_v4::drawOpcode(byte i, FuncParams &params) {
	FuncType type = _opcodesDrawV4[i].type;
	params.desc = _opcodesDrawV4[i].desc;
	OpcodeDrawProcV4 op = _opcodesDrawV4[i].proc;

	if (type == TYPE_NONE)
		warning("No such opcodeDraw: %d", i);
	if ((type == TYPE_TEXTDESC) || (type == TYPE_BOTHDESC))
		printFuncDesc(params, _opcodesDrawV4[i].params);
	if ((type == TYPE_FUNCDESC) || (type == TYPE_BOTHDESC))
		(this->*op)(params);
}

void Script_v4::funcOpcode(byte i, byte j, FuncParams &params) {
	int n = i*16 + j;
	FuncType type = TYPE_NONE;
	OpcodeFuncProcV4 op = 0;

	if ((i <= 4) && (j <= 15)) {
		op = _opcodesFuncV4[n].proc;
		params.desc = _opcodesFuncV4[n].desc;
		type = _opcodesFuncV4[n].type;
	}

	if (type == TYPE_NONE)
		error("No such opcodeFunc: %d.%d", i, j);
	if ((type == TYPE_TEXTDESC) || (type == TYPE_BOTHDESC))
		printFuncDesc(params, _opcodesFuncV4[n].params);
	if ((type == TYPE_FUNCDESC) || (type == TYPE_BOTHDESC))
		(this->*op)(params);
}

void Script_v4::goblinOpcode(int i, FuncParams &params) {
	int n = -1;
	...
	if (n >= 0) {
		op = _opcodesGoblinV4[n].proc;
		params.desc = _opcodesGoblinV4[n].desc;
		type = _opcodesGoblinV4[n].type;
	}
	...
	if ((type == TYPE_TEXTDESC) || (type == TYPE_BOTHDESC)) {
		const Param *param = _opcodesGoblinV4[n].params;
		if (*param == PARAM_GOB)
